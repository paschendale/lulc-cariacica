...
dataset_media_b5 <- raster(paste('./',resolucao,'/','media_2009_b5','.tif',sep=''))
...
sink(file = paste('./stats/',dataset,'_nir_stats.txt',sep=''))
cm
sink(file = NULL)
...
raster::writeRaster(pred,paste('./classes/class_',dataset,'_nir.tif',sep=''),overwrite=TRUE)
toc()
